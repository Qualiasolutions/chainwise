...
-- Portfolios policies
CREATE POLICY "Users can view own portfolios" ON public.portfolios FOR SELECT USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can insert own portfolios" ON public.portfolios FOR INSERT WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "Users can update own portfolios" ON public.portfolios FOR UPDATE USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can delete own portfolios" ON public.portfolios FOR DELETE USING ((select auth.uid()) = user_id);

-- Portfolio holdings policies
CREATE POLICY "Users can view own holdings" ON public.portfolio_holdings FOR SELECT USING (
    (select auth.uid()) IN (SELECT user_id FROM public.portfolios WHERE id = portfolio_id)
);
...
CREATE POLICY "Users can view own chat sessions" ON public.ai_chat_sessions FOR SELECT USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can insert own chat sessions" ON public.ai_chat_sessions FOR INSERT WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "Users can update own chat sessions" ON public.ai_chat_sessions FOR UPDATE USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can delete own chat sessions" ON public.ai_chat_sessions FOR DELETE USING ((select auth.uid()) = user_id);

-- Chat messages policies
CREATE POLICY "Users can view own chat messages" ON public.ai_chat_messages FOR SELECT USING (
    (select auth.uid()) IN (SELECT user_id FROM public.ai_chat_sessions WHERE id = session_id)
);
...
CREATE POLICY "Users can view own transactions" ON public.credit_transactions FOR SELECT USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can insert own transactions" ON public.credit_transactions FOR INSERT WITH CHECK ((select auth.uid()) = user_id);
...
CREATE POLICY "Users can insert own alerts" ON public.user_alerts FOR INSERT WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "Users can update own alerts" ON public.user_alerts FOR UPDATE USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can delete own alerts" ON public.user_alerts FOR DELETE USING ((select auth.uid()) = user_id);

-- Subscriptions policies
CREATE POLICY "Users can view own subscriptions" ON public.subscriptions FOR SELECT USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can insert own subscriptions" ON public.subscriptions FOR INSERT WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "Users can update own subscriptions" ON public.subscriptions FOR UPDATE USING ((select auth.uid()) = user_id);

-- Health checks policies (system monitoring - read-only for all authenticated users)
CREATE POLICY "Health checks are viewable by authenticated users" ON public.health_checks FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Health checks can be updated by service role" ON public.health_checks FOR ALL USING (auth.role() = 'service_role');
