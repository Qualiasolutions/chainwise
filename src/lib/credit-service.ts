...
export class CreditService {
  ...
  // Credit packs from source of truth
  static readonly CREDIT_PACKS: CreditPack[] = [
    {
      id: 'pack_50',
  ...
  getCreditPacks(): CreditPack[] {
    return CreditService.CREDIT_PACKS
  }

  /**
   * Get specific credit pack by ID
   */
  getCreditPack(packId: string): CreditPack | null {
    return CreditService.CREDIT_PACKS.find(pack => pack.id === packId) || null
  }
  ...
  async addCredits(
    userId: string, 
    amount: number, 
    transactionType: 'granted' | 'purchased' = 'granted',
    description?: string,
    metadata?: Record<string, any>
  ): Promise<{ newBalance: number }> {
  ...
  async getUserCreditAnalytics(
    ...
    const totalEarned = transactions
      .filter(t => ['granted', 'purchased'].includes(t.transaction_type))
