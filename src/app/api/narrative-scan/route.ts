...
async function generateNarrativeReport(
  ...
  const narratives = await generateRealNarratives(scanType, specificNarrative, timeframe)
  const tokens = includeTokens ? await generateRelatedTokens(narratives) : []
...
async function generateRealNarratives(scanType: string, specific: string | undefined, timeframe: string) {
